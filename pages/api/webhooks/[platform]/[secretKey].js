import { getGameBySecretKey, incrementGameStats, addDonation } from '../../../../lib/db';

export default async function handler(req, res) {
  if (req.method !== 'POST') {
    return res.status(405).json({ error: 'Method not allowed' });
  }

  try {
    const { platform, secretKey } = req.query;
    const body = req.body;

    let donorName, donationAmount, donationMessage;

    if (platform === 'saweria') {
      donorName = body.name;
      donationAmount = body.amount;
      donationMessage = body.message;
    } else if (platform === 'bagibagi') {
      donorName = body.name || body.supporter_name || body.donor_name;
      donationAmount = body.amount;
      donationMessage = body.message || body.comment;
    } else {
      return res.status(400).json({ error: 'Invalid platform' });
    }

    const game = await getGameBySecretKey(secretKey);
    if (!game) {
      return res.status(404).json({ error: 'Game not found' });
    }

    if (!donorName || !donationAmount || donationAmount <= 0) {
      return res.status(400).json({ error: 'Invalid donation data' });
    }

    const donation = {
      id: `don_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      game: game.name,
      donor: donorName,
      amount: parseInt(donationAmount),
      message: donationMessage || '',
      platform: platform,
      timestamp: new Date().toISOString()
    };

    await addDonation(donation);
    await incrementGameStats(game.id, parseInt(donationAmount));

    console.log(`[${platform.toUpperCase()}] Donation for "${game.name}": ${donorName} → Rp ${donationAmount}`);

    res.status(200).json({
      success: true,
      donation,
      game: {
        name: game.name,
        robloxGameId: game.robloxGameId
      }
    });
  } catch (err) {
    console.error('Webhook error:', err);
    res.status(500).json({ error: 'Server error' });
  }
}
